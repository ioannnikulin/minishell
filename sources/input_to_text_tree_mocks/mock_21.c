...
static t_treenode	*p1(t_treenode *cur, t_treenode *root)
{
	...
	if (treenode_insert_dup(cur, "1", 0) == -1
		...
		|| treenode_insert_dup(root, "(", 2) == -1
	)
		return (0);
	cur = cur->sibling_next->sibling_next;
	if (treenode_insert_dup(cur, "echo", 0) == -1
		|| treenode_insert_dup(cur->child, "2", 0) == -1
		|| treenode_insert_dup(cur, "&&", 1) == -1
		|| treenode_insert_dup(cur, "(", 2) == -1
	)
		return (0);
	return (cur->child->sibling_next->sibling_next);
}

static t_treenode	*p2(t_treenode *cur)
{
	if (treenode_insert_dup(cur, "echo", 0) == -1
		|| treenode_insert_dup(cur->child, "3", 0) == -1
		|| treenode_insert_dup(cur, "&&", 1) == -1
		|| treenode_insert_dup(cur, "(", 2) == -1
		|| treenode_insert_dup(cur, "||", 3) == -1
		|| treenode_insert_dup(cur, "echo", 4) == -1
		|| treenode_insert_dup(cur, "&&", 5) == -1
		|| treenode_insert_dup(cur, "echo", 6) == -1
	)
		return (0);
	cur = cur->child->sibling_next->sibling_next;
	if (treenode_insert_dup(cur, "echo", 0) == -1
		|| treenode_insert_dup(cur->child, "4", 0) == -1
	)
		return (0);
	cur = cur->sibling_next->sibling_next;
	if (treenode_insert_dup(cur, "5", 0) == -1)
		return (0);
	cur = cur->sibling_next->sibling_next;
	if (treenode_insert_dup(cur, "6", 0) == -1)
		return (0);
	return (cur);
}
...
{
	return (mock_21_tree(root));
}

/* returns 1 on success - to allow condition usage outside */
int	mock_21(char *input, t_treenode *root, int *errno)
{
	if (ft_strcmp(input, MOCK_21_TEXT) != 0 && ft_strcmp(input, "MOCK_21") != 0)
		return (0);
	if (mock_21_tree(root))
