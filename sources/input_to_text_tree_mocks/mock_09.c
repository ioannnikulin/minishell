...
{
	if (treenode_insert_dup(cur, "export", 0) == -1
		|| treenode_insert_dup(cur->child, "foo=bar", 0) == -1
		|| treenode_insert_dup(cur, "&&", 1) == -1
		|| treenode_insert_dup(cur, "export", 2) == -1)
		return (1);
	cur = cur->child->sibling_next->sibling_next;
	if (treenode_insert_dup(cur, "foo=zah", 0) == -1
		|| treenode_insert_dup(cur, "nope=uhoh", 1) == -1
		|| treenode_insert_dup(cur->parent, "&&", 3) == -1
		|| treenode_insert_dup(cur->parent, "unset", 4) == -1)
		return (1);
	cur = cur->sibling_next->sibling_next;
	if (treenode_insert_dup(cur, "nope", 0) == -1
		|| treenode_insert_dup(cur->parent, "&&", 5) == -1
		|| treenode_insert_dup(cur->parent,
			"./tests/tool_print_environment", 6) == -1)
		return (1);
	cur = cur->sibling_next->sibling_next;
	if (treenode_insert_dup(cur, "one", 0) == -1
		|| treenode_insert_dup(cur, "\"two   three\"", 1) == -1
		|| treenode_insert_dup(cur, "four", 2) == -1)
		return (1);
	return (0);
}
