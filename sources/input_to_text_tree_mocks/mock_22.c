...
static t_treenode	*p1(t_treenode *cur, t_treenode *root)
{
	...
	if (treenode_insert_dup(cur, "6", 0) == -1)
		return (0);
	cur = cur->sibling_prev->sibling_prev->child->sibling_next->sibling_next;
	return (cur);
}
...
{
	return (mock_22_tree(root));
}

/* returns 1 on success - to allow condition usage outside */
int	mock_22(char *input, t_treenode *root, int *errno)
{
	if (ft_strcmp(input, MOCK_22_TEXT) != 0 && ft_strcmp(input, "MOCK_22") != 0)
		return (0);
	if (mock_22_tree(root))
