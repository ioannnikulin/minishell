...
int	populate(void)
{
	...
	empty_test = (t_testcase){"exit", empty_m, 0};
	...
	tests[29] = (t_testcase){"rm -f one.txt && echo 1 > one.txt && cat < one.txt", m[29], 0};

	ft_mapss_add(m[30], "stdout", "");
	ft_mapss_add(m[30], "one.txt", "1");
	ft_mapss_add(m[30], "two.txt", "");
	ft_mapss_add(m[30], "stderr", "minishell: two.txt: No such file or directory\n");
	tests[30] = (t_testcase){"rm -f one.txt two.txt && echo 1 > one.txt && cat < two.txt < one.txt", m[30], 1};

	ft_mapss_add(m[31], "stdout", "1");
	ft_mapss_add(m[31], "one.txt", "1\n2\n");
	tests[31] = (t_testcase){"rm -f one.txt && echo 1 >> one.txt && echo 2 >> one.txt && cat < one.txt | grep 1", m[31], 0};

	ft_mapss_add(m[32], "stdout", "");
	ft_mapss_add(m[32], "one.txt", "1\n2\n");
	ft_mapss_add(m[32], "one.txt", "2\n");
	tests[32] = (t_testcase){"rm -f one.txt two.txt && echo 1 >> one.txt && echo 2 >> one.txt && cat < one.txt | grep 2 > two.txt", m[32], 0};

	ft_mapss_add(m[33], "stdout", "");
	ft_mapss_add(m[33], "one.txt", "1\n2\n");
	ft_mapss_add(m[33], "one.txt", "2\n");
	tests[33] = (t_testcase){"rm -f one.txt two.txt && echo 1 >> one.txt && echo 2 >> one.txt && cat < one.txt | grep 2 > two.txt | grep 2", m[33], 0};
