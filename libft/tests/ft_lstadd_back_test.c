...
/*   Updated: 2023/11/21 21:14:10 by inikulin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_lstadd_back_test(void)
{
	...
	for (int i = 0; i <  SZ; i ++)
	{
		...
		t_list *root = n;
		t_list *nnode = (t[i].toadd ? ft_lstnew(ft_strdup(t[i].toadd)) : NULL);
		ft_lstadd_back(&n, nnode);
		t_list *orig = ft_lst_generate(t[i].texts, t[i].end_to);
