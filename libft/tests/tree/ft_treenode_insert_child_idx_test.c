...
/*   Updated: 2024/11/08 13:03:45 by inikulin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_treenode_insert_child_idx_test(void)
{
	...
	for (int i = 0; i < SZ; i ++) {
		...
		ft_printf("%i: ", i);
		#endif
		int ret = ft_treenode_insert_child_idx(tgt, ft_treenode_make(ft_new_pi_pvoid(t[i].value), 0, 1, &ft_free_pi_null), t[i].idx);
