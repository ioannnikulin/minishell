...
void	ft_ulltoa_base_test(void)
{
	...
	t[15] = (t_testcase){LLONG_MAX, "0123456789abcde0", ""};
	t[16] = (t_testcase){LLONG_MAX, "0123456789abcde+", ""};
	t[17] = (t_testcase){LLONG_MAX, "0", ""};
	for (int i = 0; i <  SZ; i ++)
	{
		char res[MX];
		size_t sz = ft_ulltoa_base(t[i].val, t[i].base, res);
		#ifdef DEBUG
